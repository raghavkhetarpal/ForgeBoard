...
- **Label Deletion**: Deleting a label silently cascades to remove it from all issues it is currently attached to via the join table (no "label is in use" guard). This is an intentional design choice for administrative simplicity.
- **Comment Moderation**: Editing a comment is strictly restricted to its author. Deleting a comment is restricted to its author OR any user with `ADMIN`+ role in the project to allow for basic moderation.
- **Notifications & Side-effects**: Notifications are best-effort side-effects. Notification creation happens outside the primary transaction, wrapped in a `try/catch`. Failure to create a notification will be logged but will never block the primary action (like creating a comment).
- **Mentions Parsing**: Mentions in comments use the `@email` syntax (e.g. `@alice@example.com`). Mentioning a non-project-member is silently ignored (does not trigger a notification and does not reject the comment). Self-mentions are similarly ignored.
